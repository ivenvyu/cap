...
DEFAULT_SCORE_SNAPSHOT = "data/retrieval/phase1b/v2_2_2/candidate_score_snapshot_v2_2_2.jsonl"
DEFAULT_FEATURE_GLOB = "data/feature_snapshots/v2_2_2/phase1b_dinov2_anchor/*.jsonl"
DEFAULT_CAMPAIGN_BRIEFS = "configs/campaign_review_briefs_v1.yaml"
DEFAULT_OUT_DIR = "data/review/phase1b/v2_2_4/shortlist"
...
def get_score(row: dict[str, Any]) -> float:
    ...
    for container_key in ["scores", "model_scores", "score_snapshot"]:
        ...
        if isinstance(obj, dict):
            ...
            # common diagnostic probability names
            for k in [
            ...
            # if dict has one numeric value, use it
            numeric_items = []
            ...
            # prefer keys that look like positive/prob/score
            preferred = [
                item for item in numeric_items
                if any(token in item[0].lower() for token in ["positive", "prob", "score", "accept"])
            ]
            if preferred:
    ...
    # 3. fallback: any top-level numeric score/prob key
...
<body>
  <h1>{esc(title)}</h1>
  <div class="notice">
    <b>v2.2.4 Recommendation Shortlist</b><br>
...
def main() -> None:
    ...
    csv_out = out_dir / "recommendation_shortlist_v2_2_4.csv"
    labeled_template = out_dir / "recommendation_shortlist_v2_2_4.labeled_template.csv"
    ...
    for cid, cr in sorted(by_campaign.items()):
        ...
        page_name = f"recommendation_shortlist_v2_2_4__{cid}.html"
...
<head>
  <meta charset="utf-8" />
  <title>v2.2.4 Recommendation Shortlist</title>
  <style>
    body {{
      font-family: -apple-system, BlinkMacSystemFont, "Apple SD Gothic Neo", "Noto Sans KR", sans-serif;
      margin: 32px;
...
<body>
  <h1>v2.2.4 Recommendation Shortlist</h1>
...
"""
    ...
    summary_out = out_dir / "recommendation_shortlist_v2_2_4_summary.json"
